...
// Configuration
const GITHUB_USERNAME = 'zypheriss'; // GitHub kullanıcı adınızı buraya yazın
const DISCORD_USER_ID = '773582512647569409'; // Discord kullanıcı ID'nizi buraya yazın
...
(function() {
    ...
    setInterval(function() {
        if (window.outerHeight - window.innerHeight > threshold || 
            ...
            if (!devtools.open) {
                devtools.open = true;
                // Redirect to homepage if dev tools detected
...
async function loadProjects() {
    try {
        ...
        // Load featured projects (first 5) for homepage
        if (projectsGrid && projectsLoading) {
            ...
            // Add "More Projects" card as 6th item
        ...
        if (allProjectsGrid && allProjectsLoading) {
            ...
            allProjectsGrid.innerHTML = filteredRepos.map((project, index) => 
                createProjectCard(project, index === 0)).join('');
        }
